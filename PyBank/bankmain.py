...
PyBank = open("Financial_Summary_Statement.txt", "w+")
PyBank.write("Financial Summary Statement")
PyBank.write('\n' + "------------------------------")
PyBank.write('\n' + f"Total Months: {len(total_months)}")
PyBank.write('\n' + f"Tota Net Profit: ${sum(profit_total)}")
PyBank.write('\n' + f"Average Change in Profit: ${round(sum(monthly_change)/len(monthly_change),2)}")
PyBank.write('\n' + f"Greatest Increase in Profits: {total_months[month_max]} (${(str(greatest_increase))})")
PyBank.write('\n' + f"Greatest Decrease in Profits: {total_months[month_min]} (${(str(greatest_decrease))})")
PyBank.write('\n' + "------------------------------")
